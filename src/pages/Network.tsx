...
const Network: React.FC = () => {
  ...
  const [dataSource, setDataSource] = useState<Array<{ w_id: string, worker_url: string, last_seen: string, created_at: string }>>([]);
    ...
    let interval = setInterval(() => {
      ...
      fetch(worker_list_api_url, { signal: controller.signal, headers: { 'api-token': currentUser.api_token } }).then((res) => res.json()).then((data) => {
        ...
        data = data.workers.map((item: any) => {
          return {
            'w_id': `${item.nickname ? item.nickname : "No name"} (${item.worker_id}) owned by ${item.owner_email}`,
            'worker_url': item.address,
  ...
  return (
    <div>
      ...
      <Card>
        ...
        <p>
          The Golden Fleece Network is a decentralized network of workers that process requests from the Golden Fleece API. 
          The network is currently in alpha. 
        </p>
      </Card>
      <br />
      ...
      <Card title={
        ...
        <Table dataSource={dataSource} columns={[
          ...
          },
          {
            ...
            'render': (v) => {
              return (<Tag icon={<CheckCircleOutlined />} color="default">
                {v}
              </Tag>)
            }
          },
